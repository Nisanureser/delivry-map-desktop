...
interface DeliveryPointDetailFormProps {
  point: DeliveryPoint;
  onClose: () => void;
  onSave: (id: string, updates: { priority: Priority; notes?: string }) => void;
}

function DeliveryPointDetailForm({
  point,
  onClose,
  onSave,
}: DeliveryPointDetailFormProps) {
  const [priority, setPriority] = useState<Priority>(point.priority);
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {/* Öncelik Seçimi */}
        <div>
          <label className={`block mb-3 ${sectionTitleClass}`}>Öncelik</label>
        ...
        {/* Not/Açıklama */}
        <div>
          <label htmlFor="notes" className={`block mb-2 ${sectionTitleClass}`}>
            Not / Açıklama
          </label>
          <div className="relative">
            <div className="absolute left-3 top-3 pointer-events-none">
